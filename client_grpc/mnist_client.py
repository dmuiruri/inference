...
def _create_rpc_callback(label, result_counter, start_time):
  ...
  def _callback(result_future):
    ...
    if exception:
      ...
      elapsed_time.append(response_time)
...
def do_inference(hostport, work_dir, concurrency, num_tests):
  ...
  for _ in range(num_tests):
    ...
    result_future.add_done_callback(
        _create_rpc_callback(label[0], result_counter, start_time))
...
def main(_):
  ...
  for concurrent_reqs in [500]: #100, 200, 300, 500, 1000, 1500, 2000, 2500, 3000, 3500, 4000
    ...
    res_df[concurrent_reqs] = pd.Series(elapsed_time)
    elapsed_time.clear()
    #numpy.save('./data/response_times', elapsed_time)
  res_df.to_csv('./data/response_times.csv')
